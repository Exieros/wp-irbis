...
function stringToKeywordExpression( string ) {
	...
	if ( 0 == keywords_array.length ) {
		return null;
	}

	let expression = '((';
	for ( let i in keywords_array ) {
		expression += ( 0 == i ) ? `K=${keywords_array[i]}$` :  ` * K=${keywords_array[i]}$`;
	}
	expression += ')';
	expression += ` + K=${string})`;
...
jQuery( '#irbis .searchbar__form' ).keypress( function( e ) {
	if ( 13 == e.which ) {
		let value = jQuery( '.wp_irbis__searchbar input' ).val();
		let clear_val = value.split( ' ' );
		value = clear_val.filter( a=>{
			return '' != a;
		}).join( ' ' );
...
function formFields( fields, dictionaries ) {
	if ( null === wp_irbis_output_template ) {
			...
			for ( let _if of condition.if ) {
				if ( 'string' === typeof _if ) {
					...
					if ( matches_ve ) {
						...
						let __f = f.filter( _f=>{
							return 0 != _f.subfields.filter( subf=>{
								return subf.code == b;
							}).length;
						});
						if ( 0 === __f.length ) {
